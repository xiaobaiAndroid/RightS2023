 ...
 * @author: 78495
*/
public class InteriorLightingFragment extends CarLightingFragmentBase{
    ...
    protected void updateData(int position) {
        mExecutor.execute(() -> {
            ...
            send = SerialPortDataFlag.START_FLAG + SerialPortDataFlag.INTERIOR_LIGHTING.getTypeValue() +  status + SerialPortDataFlag.END_FLAG;
    ...
    protected void setupData() {
        ...
        mExecutor.execute(() -> {
            ...
            if(carSetupTable!=null){
                if (carSetupTable.getInnerLighting().equals(Can35dStatus.D2.DELAY_0.getValue())) {
                    currentPosition = 0;
                } else if (carSetupTable.getInnerLighting().equals(Can35dStatus.D2.DELAY_15.getValue())) {
                    currentPosition = 1;
                } else if (carSetupTable.getInnerLighting().equals(Can35dStatus.D2.DELAY_30.getValue())) {
                    currentPosition = 2;
                } else if (carSetupTable.getInnerLighting().equals(Can35dStatus.D2.DELAY_45.getValue())) {
                    currentPosition = 3;
                } else if (carSetupTable.getInnerLighting().equals(Can35dStatus.D2.DELAY_60.getValue())) {
                    currentPosition = 4;
                }
