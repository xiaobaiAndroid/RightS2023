...
//				}
    ...
    public static List<SiriCommandEntity> parseGrammarResult(String json) {

        List<SiriCommandEntity> list= new ArrayList<>();
        try {
            JSONTokener tokener = new JSONTokener(json);
            JSONObject joResult = new JSONObject(tokener);
            ...
            // 云端和本地结果分情况解析
            for (int i = 0; i < words.length(); i++) {
                SiriCommandEntity entity = new SiriCommandEntity();
...
//                } else {
                    ...
                    if (!obj.getString("w").contains("nomatch")) {
                        ...
                        entity.setText(obj.getString("w"));
                        int reliability = Integer.parseInt(obj.getString("sc"));
